...
export default function MultipleBooking() {
  ...
  const [stayType, setStayType] = useState<"short_stay" | "overnight">(
    "overnight",
  );
  ...
  const calculateRoomSubtotal = (
    room: Room,
    stayTypeValue: "short_stay" | "overnight",
    checkIn: Date,
    checkOut: Date,
    hours?: number,
  ) => {
    const pricePerUnit =
      stayTypeValue === "short_stay"
        ? room.room_type?.short_stay_price || room.room_type?.base_price || 0
  ...
  const addRoom = () => {
    ...
    const nights =
      stayType === "short_stay" ? 1 : getNightsCount(checkInDate, checkOutDate);
    ...
    setSelectedRoomsDetails((prev) => [
      ...prev,
      {
        ...
        stay_type: stayType,
        check_in_date: formatLocalDate(checkInDate),
        check_out_date: formatLocalDate(checkOutDate),
        nights: nights,
        subtotal: Number(subtotal),
        hours: stayType === "short_stay" ? 3 : undefined,
      },
    ]);
  ...
  const handleBooking = () => {
    ...
    router.push({
      ...
      params: {
        ...
        rooms: JSON.stringify(selectedRoomsDetails),
      },
    });
  };
      ...
      >
        <View className="flex-row items-center justify-between mb-6">
          ...
          <TouchableOpacity
            ...
            className="px-3 py-2"
          >
            <Text className="text-[#c9a96e] text-xs tracking-wider">
              Clear All
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <Text
          className="text-white/40 text-sm tracking-wide mt-1"
      ...
      >
        ...
        <View className="px-6 mt-8">
          ...
          {selectedRoomsDetails.length === 0 ? (
            <View className="items-center py-12 bg-white rounded-2xl border border-[#1a4a35]/10">
              ...
              <Text className="text-[#1a4a35]/40 mt-3 text-sm">
                No rooms added yet
              </Text>
              <Text className="text-[#1a4a35]/30 text-xs mt-1">
                Tap "Add Room" to start
              </Text>
            </View>
          ) : (
                ...
                >
                  <View className="flex-row justify-between items-start">
                    <View className="flex-row items-center gap-3 flex-1">
                      ...
                      <View className="flex-1">
                        <Text
                          className="text-[#1a4a35] text-lg font-bold"
                          style={{ fontFamily: "Georgia" }}
                        >
                  ...
                  <View className="mt-3 pt-3 border-t border-[#1a4a35]/10">
                    <View className="flex-row justify-between mb-2">
                      ...
                      <View
                        className={`px-2 py-1 rounded-full ${room.stay_type === "short_stay" ? "bg-orange-100" : "bg-blue-100"}`}
                      >
                        <Text
                          className={`text-xs ${room.stay_type === "short_stay" ? "text-orange-600" : "text-blue-600"}`}
                        >
                          {room.stay_type === "short_stay"
                            ? `Short Stay (${room.hours || 3} hrs)`
                    ...
                    <View className="flex-row justify-between">
                      ...
                      <Text className="text-[#1a4a35] text-xs">
                        {formatPrice(room.price_per_unit)}
                        {room.stay_type === "overnight" && "/night"}
                      </Text>
                    </View>
        ...
        {selectedRoomsDetails.length > 0 && (
          <View className="mx-6 mt-6 p-5 rounded-2xl bg-white shadow-lg border border-[#1a4a35]/10">
            <Text
              className="text-[#1a4a35] text-sm uppercase tracking-wider mb-3"
              style={{ fontFamily: "Georgia" }}
            >
              Booking Summary
            </Text>
            <View className="gap-2">
              <View className="flex-row justify-between">
                ...
                <Text className="text-[#1a4a35] font-medium">
                  {selectedRoomsDetails.length} room(s)
                </Text>
              </View>
              <View className="flex-row justify-between">
                ...
                <Text className="text-[#1a4a35] font-medium">
                  {
                    selectedRoomsDetails.filter(
                      (r) => r.stay_type === "short_stay",
                    ).length
                  }
                </Text>
              </View>
        ...
        <View className="flex-1 bg-black/50 justify-end">
          ...
          >
            ...
            <ScrollView className="p-6" showsVerticalScrollIndicator={false}>
              {/* Room Selection */}
              <View className="mb-6">
                <Text className="text-[#1a4a35] font-medium mb-2">
                  Select Room
                </Text>
                <ScrollView
                  horizontal
                        ...
                        <TouchableOpacity
                          ...
                          className={`mb-3 p-4 rounded-2xl border ${
                            selectedRoomId === room.id
                              ...
                              : "border-[#1a4a35]/10 bg-white"
                          }`}
                          style={{ minWidth: width - 48 }}
                        >
                          <View className="flex-row justify-between items-center">
                            <View>
                              <Text className="text-[#1a4a35] font-bold text-lg">
                                Room {room.room_number}
                              </Text>
                              <Text className="text-[#1a4a35]/50 text-xs">
                                {typeName}
                            ...
                            <View className="items-end">
                              <Text className="text-[#c9a96e] font-bold">
                                {formatPrice(room.room_type?.base_price || 0)}
                                /night
                              </Text>
                              {room.room_type?.short_stay_price && (
                                <Text className="text-[#1a4a35]/40 text-xs">
                                  or{" "}
                                  {formatPrice(room.room_type.short_stay_price)}{" "}
                                  short stay
                                </Text>
                              )}
                            </View>
                ...
                {rooms.length === 0 && !fetchingRooms && (
                  <Text className="text-[#1a4a35]/40 text-center py-8">
                    No available rooms
                  </Text>
                )}
              </View>
              ...
              <View className="mb-6">
                ...
                <View className="flex-row gap-3">
                  ...
                  >
                    <Text
                      className={`text-center ${stayType === "overnight" ? "text-white" : "text-[#1a4a35]"}`}
                    >
                      Overnight
                    </Text>
                  </TouchableOpacity>
                  ...
                  >
                    <Text
                      className={`text-center ${stayType === "short_stay" ? "text-white" : "text-[#1a4a35]"}`}
                    >
                      Short Stay
                    </Text>
                  </TouchableOpacity>
                ...
                <View className="mb-6 p-4 rounded-xl bg-[#c9a96e]/10 border border-[#c9a96e]/20">
                  <View className="flex-row justify-between items-center">
                    <Text className="text-[#1a4a35] font-medium">
                      {stayType === "short_stay"
                        ? "Short Stay Amount:"
                        : `Total for ${getNightsCount(checkInDate, checkOutDate)} night(s):`}
                    </Text>
                    <Text
