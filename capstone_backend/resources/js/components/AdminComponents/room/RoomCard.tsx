...
const RoomCard = memo(({ room, onEdit, onDelete, onView }: any) => {
    ...
    const getStatusColor = (status: string) => {
        switch (status?.toLowerCase()) {
            case "available":
                return "bg-[#2e7d64] hover:bg-[#276b56]";
    ...
    const getEditButtonColor = (status: string) => {
        switch (status?.toLowerCase()) {
            ...
            case "reserved":
                return "bg-yellow-50 hover:bg-yellow-100 text-yellow-700 hover:text-yellow-800";
            ...
            case "dirty":
                return "bg-purple-50 hover:bg-purple-100 text-purple-700 hover:text-purple-800";

            case "cleaning":
                return "bg-amber-50 hover:bg-amber-100 text-amber-700 hover:text-amber-800";
    ...
    return (
        <>
            <div
                className={`group ${cardBgClass} rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border flex flex-col h-full`}
            >
                {/* IMAGE SECTION */}
                <div className="relative w-full h-48 overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200">
                    ...
                    <div className="absolute top-3 left-3">
                        <span
                            className={`px-2.5 py-1 text-xs font-semibold rounded-full ring-1 ${getStatusBadgeStyle(room.status)} backdrop-blur-sm bg-opacity-90 shadow-sm`}
                        >
                            {room.status?.charAt(0).toUpperCase() +
                                room.status?.slice(1) || "Unknown"}
                        </span>
                    </div>
                    ...
                    <div className="absolute bottom-3 right-3">
                        <button
                            ...
                            className={`bg-white/90 hover:bg-white backdrop-blur-sm p-2 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 hover:scale-105 ${
                                hasPanorama
                ...
                <div className="p-4 flex-1 flex flex-col">
                    ...
                    <div className="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                        ...
                        <button
                            ...
                            className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 bg-red-50 hover:bg-red-100 text-red-700 hover:text-red-800 rounded-xl transition-all text-sm font-medium"
                        >
                            <svg
                                className="w-4 h-4"
                                fill="none"
