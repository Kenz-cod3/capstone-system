...
export default function GuestDetailModal({
    ...
    const bookingColumns: ColumnsType<BookingDetails> = [
        ...
        },
        {
            ...
            render: (_, r) => (
                <div className="flex flex-col">
                    <span className="text-[#1a1a18] text-sm">{dayjs(r.check_in_date).format("MMM DD, YYYY")}</span>
                    {r.check_in_time && <span className="text-[#8a8878] text-xs">{dayjs(r.check_in_time).format("hh:mm A")}</span>}
                </div>
            ),
        },
        {
            ...
            render: (_, r) => (
                <div className="flex flex-col">
                    <span className="text-[#1a1a18] text-sm">{r.check_out_date ? dayjs(r.check_out_date).format("MMM DD, YYYY") : "—"}</span>
                    {r.check_out_time && <span className="text-[#8a8878] text-xs">{dayjs(r.check_out_time).format("hh:mm A")}</span>}
                </div>
            ),
        },
        {
            ...
            key: "stay_type",
            render: (_, r) => (
                <span className={`inline-flex px-2 py-0.5 rounded-md text-xs font-semibold ${r.stay_type === "overnight" ? "bg-[#e8f0ff] text-[#3b5bdb]" : "bg-[#fef8e1] text-[#9a6e00]"
                    }`}>
                    {r.stay_type === "overnight" ? "Overnight" : "Short Stay"}
                </span>
            ),
        },
        {
            ...
            render: (_, r) => (
                <div className="flex flex-wrap gap-1">
                    {(r.booked_rooms ?? []).map((br) => (
                        <span key={br.id} className="inline-flex px-2 py-0.5 rounded-md text-xs font-semibold bg-purple-100 text-purple-700">
        ...
        },
        {
            ...
            key: "status",
            render: (_, r) => (
                <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${STATUS_CLASSES[r.booking_status] || "bg-gray-100 text-gray-600"}`}>
                    {STATUS_LABELS[r.booking_status] || r.booking_status}
                </span>
            ),
        },
    ];
                                                            ...
                                                            <Descriptions.Item label="Rooms">
                                                                <div className="flex flex-wrap gap-1">
                                                                    {(record.booked_rooms ?? []).map((br) => (
                                                                        <span key={br.id} className="px-2 py-0.5 rounded-md text-xs font-semibold bg-purple-100 text-purple-700">
                                                                    ...
                                                                    <div className="flex flex-wrap gap-1">
                                                                        {record.add_ons.map((a) => (
                                                                            <span key={a.id} className="px-2 py-0.5 rounded-md text-xs font-semibold bg-orange-100 text-orange-700">
                                                                                {a.add_on_name} ×{a.pivot.quantity} = ₱{a.pivot.subtotal}
                                                                            </span>
                                                                        ))}
                                                                    </div>
                                                                </Descriptions.Item>
