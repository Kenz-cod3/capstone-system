...
const StatusPill = ({ status }: { status: string }) => (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold border ${statusStyles[status] || 'bg-slate-50 text-slate-500 border-slate-200'}`}>
...
export default function WalkInGuests() {
    ...
    const fetchGuests = async (silent = false) => {
        try {
            ...
            console.error(err);
            message.error(err.response?.data?.message || "Failed to load walk-in guests");
        } finally {
    ...
    const handleDeleteGuest = async (guest: WalkInGuest) => {
        try {
            ...
            message.error(error.response?.data?.message || "Failed to delete guest");
        }
    };
    ...
    const columns: ColumnsType<WalkInGuest> = [
            ...
            render: (_, record) => (
                <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                        <p className="relative top-2 font-semibold text-slate-800 text-sm truncate">{record.full_name}</p>
        ...
        },
        {
            ...
            width: 140,
            render: (_, record) => (
                <span className="font-bold text-slate-800 text-sm">{formatCurrency(record.total_spent || 0)}</span>
            ),
        },
        {
            title: "First Visit",
                    ...
                    menu={{
                        items: [
                            ...
                            { type: "divider" },
                            {
                                ...
                                icon: <DeleteOutlined />,
                                danger: true,
                                onClick: () => { setSelectedGuest(record); setDeleteModalVisible(true); },
                            },
                        ],
                    }}
                    trigger={["click"]}
    ...
    const bookingColumns: ColumnsType<BookingDetails> = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <div className="flex flex-col">
                    <span className="text-slate-700 text-sm">{dayjs(record.check_in_date).format("MMM DD, YYYY")}</span>
                    {record.check_in_time && (
                        <span className="text-slate-400 text-xs">{dayjs(record.check_in_time).format("hh:mm A")}</span>
                    )}
                </div>
            ),
        },
        ...
        },
        {
            ...
            key: "stay_type",
            render: (_, record) => (
                <span className={`inline-flex px-2.5 py-1 rounded-lg text-xs font-semibold border ${record.stay_type === "overnight"
            ...
            render: (_, record) => (
                <div className="flex flex-wrap gap-1">
                    {(record.booked_rooms ?? []).map((br) => (
                        <span key={br.id} className="inline-flex px-2.5 py-1 rounded-lg text-xs font-semibold bg-violet-50 text-violet-600 border border-violet-200">
        ...
        },
        {
            ...
            align: "right",
            render: (_, record) => (
                <span className="font-bold text-slate-800 text-sm">{formatCurrency(record.total_price)}</span>
            ),
        },
        {
            title: "Status",
            key: "status",
            align: "center",
            render: (_, record) => <StatusPill status={record.booking_status} />,
        },
    ];
        ...
        <div className="min-h-screen bg-slate-50/50 p-4 md:p-6">
                ...
                <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Walk-in Guests</h1>
                        </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                ...
                <StatCard
                    ...
                    value={total > 0 ? `₱${Math.round(totalRevenue / total).toLocaleString()}` : '₱0'}
                ...
                <Spin spinning={loading}>
                    <Table
                        ...
                        pagination={{
                            ...
                            showTotal: (t, range) => `${range[0]}-${range[1]} of ${t} guests`,
                        ...
                        locale={{
                            emptyText: (
                                <div className="py-16 flex flex-col items-center gap-3 text-slate-400">
                                    ...
                                    <p className="font-medium text-slate-500">No walk-in guests found</p>
                    ...
                    selectedGuest && (
                        <div className="flex items-center gap-4 pb-1">
                            <div
                                ...
                                style={{ backgroundColor: getAvatarColor(selectedGuest.first_name) }}
                            >
                                {getInitials(selectedGuest.first_name, selectedGuest.last_name)}
                            </div>
                            <div>
                                <p className="font-bold text-slate-800 text-lg leading-tight">{selectedGuest.full_name}</p>
                        ...
                        <div className="mt-4">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                                <div className="text-center p-3 rounded-xl bg-slate-50 border border-slate-100">
                                    ...
                                    <p className="text-[11px] text-slate-400 uppercase font-semibold tracking-wider mb-0.5">Visits</p>
                                    <p className="text-xl font-bold text-slate-800 m-0">{guestDetails.summary.total_bookings}</p>
                                </div>
                                <div className="text-center p-3 rounded-xl bg-slate-50 border border-slate-100">
                                    <span className="text-emerald-500 text-lg mb-1.5 block">₱</span>
                                    <p className="text-[11px] text-slate-400 uppercase font-semibold tracking-wider mb-0.5">Total Spent</p>
                                    <p className="text-xl font-bold text-emerald-600 m-0">{formatCurrency(guestDetails.summary.total_spent)}</p>
                                </div>
                                <div className="text-center p-3 rounded-xl bg-slate-50 border border-slate-100">
                                    <span className="text-amber-500 text-lg mb-1.5 block">₱</span>
                                    <p className="text-[11px] text-slate-400 uppercase font-semibold tracking-wider mb-0.5">Average</p>
                                ...
                                <div className="text-center p-3 rounded-xl bg-slate-50 border border-slate-100">
                                    ...
                                    <p className="text-[11px] text-slate-400 uppercase font-semibold tracking-wider mb-0.5">First Visit</p>
                            ...
                            <Tabs activeKey={activeTab} onChange={setActiveTab} className="guest-tabs">
                                                ...
                                                <div className="p-4 bg-slate-50 rounded-xl">
                                                    <Descriptions column={{ xs: 1, sm: 2 }} size="small" bordered>
                                                        ...
                                                        <Descriptions.Item label="Status">
                                                            <StatusPill status={record.booking_status} />
                                                        </Descriptions.Item>
                                                        ...
                                                        <Descriptions.Item label="Total Amount">
                                                            <span className="font-bold text-slate-800">{formatCurrency(record.total_price)}</span>
                                                        </Descriptions.Item>
                                                            ...
                                                            <Descriptions.Item label="Add-ons" span={2}>
                                                                <div className="flex flex-wrap gap-1">
                                                                    {record.add_ons.map((addon) => (
                                                                        <span key={addon.id} className="inline-flex px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-50 text-amber-600 border border-amber-200">
                                ...
                                <TabPane tab="Guest Information" key="overview">
                                    <div className="rounded-xl border border-slate-200 overflow-hidden">
                                        {[
                                            { icon: <UserOutlined />, label: 'Full Name', value: guestDetails.guest.full_name },
                                            { icon: <PhoneOutlined />, label: 'Phone', value: guestDetails.guest.contact_number || 'N/A' },
                                            { icon: <EnvironmentOutlined />, label: 'Address', value: guestDetails.guest.address || 'N/A' },
                                            { icon: <CalendarOutlined />, label: 'Member Since', value: formatDate(guestDetails.guest.created_at) },
                                            { icon: <CalendarOutlined />, label: 'Last Update', value: formatDate(guestDetails.guest.updated_at) },
                                            {
                                                icon: <StarOutlined />, label: 'Last Visit',
                                                value: guestDetails.bookings[0]?.check_in_date
                                        ...
                                        <div className="flex items-start gap-3 px-4 py-3">
                                            ...
                                            <div>
                                                <p className="text-xs text-slate-400 font-medium mb-1.5">Stay Types</p>
                                                <div className="flex gap-1.5">
                                                    {guestDetails.bookings.some(booking =>
                                                        (booking.booked_rooms ?? []).some(room =>
                                                            ...
                                                            <span className="inline-flex px-2.5 py-1 rounded-lg text-xs font-semibold bg-blue-50 text-blue-600 border border-blue-200">Overnight</span>
                                                        )}
                                                    {guestDetails.bookings.some(booking =>
                                                        (booking.booked_rooms ?? []).some(room =>
                                                            ...
                                                            <span className="inline-flex px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-50 text-amber-600 border border-amber-200">Short Stay</span>
                                                        )}
                                                </div>
                                            </div>
                                        </div>
            ...
            >
                <div className="text-center py-4">
                    ...
                    <h3 className="text-lg font-bold text-slate-800 mb-1">Delete Walk-in Guest</h3>
                    <p className="text-slate-500 text-sm mb-2">
                        Are you sure you want to permanently delete{' '}
                        <span className="font-semibold text-slate-700">{selectedGuest?.full_name}</span>?
                    </p>
                    <p className="text-xs text-red-500 bg-red-50 border border-red-200 rounded-xl px-3 py-2 mx-auto max-w-sm">
                        ⚠️ This action cannot be undone. All associated booking records will be permanently removed.
                    </p>
